...
const section = document.querySelector('.portfolio-contact'); // секция родитель
const blurElem = document.querySelector('.portfolio-contact__form-blur'); // форма
const formElem = document.querySelector('.portfolio-contact__form'); // элемент с блюром

let blurFunc = blur.bind(null, section, formElem, blurElem);

window.addEventListener('load', blurFunc);

window.addEventListener('resize', blurFunc);
